...
const assertAccount = async (address: string) => {
  ...
  const res = await db.query("SELECT address from account WHERE address=$1", [address]);
  if (res.rowCount == 0) {
...
const getNextHeight = async () => {
  ...
  try {
    const res = await client.query("SELECT * FROM block ORDER BY height DESC LIMIT 1");
    if (res.rowCount == 0) {
...
const getUnbondingHeight = async (height: bigint, unbondingPeriod: number): Promise<bigint> => {
  const db = getInstance();
  const timestamp = await db.query("SELECT * FROM block WHERE height=$1", [height]);
  if (timestamp.rowCount && timestamp.rowCount > 0) {
    ...
    const unbondingHeight = await db.query("SELECT * FROM block WHERE timestamp<=$1 ORDER BY height desc LIMIT 1", [
      unbondingTime,
    ]);
