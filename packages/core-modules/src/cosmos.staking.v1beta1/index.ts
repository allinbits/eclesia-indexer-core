...
export type Events = {
  "/cosmos.staking.v1beta1.MsgEditValidator": { value: Types.TxResult<Uint8Array> };
  "/cosmos.staking.v1beta1.MsgCreateValidator": { value: Types.TxResult<Uint8Array> };
  "/cosmos.staking.v1beta1.MsgBeginRedelegate": { value: Types.TxResult<Uint8Array> };
  "/cosmos.staking.v1beta1.MsgDelegate": { value: Types.TxResult<Uint8Array> };
  "/cosmos.staking.v1beta1.MsgUndelegate": { value: Types.TxResult<Uint8Array> };
...
export class StakingModule implements Types.IndexingModule {
  ...
  async init(pgIndexer: PgIndexer): Promise<void> {
    ...
    this.indexer.on(
      "gentx/cosmos.staking.v1beta1.MsgCreateValidator", async(event): Promise<void> => {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
          "Value passed to genesis indexing module: " + JSONbig.stringify((event as any).value)
        );
  ...
  async getUnbondingHeight(
    ...
    const db = this.pgIndexer.getInstance();
    const timestamp = await db.query("SELECT * FROM blocks WHERE height=$1", [
      height
    ]);
  ...
  async updateSlashedValidator(
    ...
    if (delegators.rowCount && delegators.rowCount > 0) {
      for (let i = 0; i < delegators.rows.length; i++) {
        ...
        const { unbondingTime } = await this.getStakingParams(height);
