...
function createGenesisIndexer(): EclesiaIndexer<CosmosAdapter> {
...
describe.sequential("Genesis import (stream-json, no database)", () => {
  bench(`Import ${ACCOUNTS} accounts + ${ACCOUNTS} balances (2 array handlers)`, async () => {
    ...
    let seen = 0;
    indexer.on("genesis/array/app_state.auth.accounts" as never, ((event: {
      value: unknown[]
    }) => {
      seen += event.value.length;
    }) as never);
    indexer.on("genesis/array/app_state.bank.balances" as never, ((event: {
      value: unknown[]
    }) => {
      seen += event.value.length;
    }) as never);
