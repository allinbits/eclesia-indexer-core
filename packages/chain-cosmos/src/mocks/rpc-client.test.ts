...
describe("MockRpcClient", () => {
  ...
  it("produces CometBFT 0.38 results tagged with the SDK's mode values", async () => {
    ...
    const events = (results as unknown as {
      finalizeBlockEvents: Parameters<typeof Utils.hasBlockEventMode>[0][]
    }).finalizeBlockEvents;
    expect(events.filter(e => Utils.hasBlockEventMode(e, "BeginBlock")).length).toBe(1);
    expect(events.filter(e => Utils.hasBlockEventMode(e, "EndBlock")).length).toBe(1);
