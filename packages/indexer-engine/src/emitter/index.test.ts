...
import { EclesiaEmitter } from "./";

test("Handling of emit types", () => {
  const emitter = new EclesiaEmitter<{ test: string;
    test2: number; }>();

  emitter.on("test", (_arg) => {
  ...
  emitter.on("test2", (_arg) => {
        
  });
  expect(emitter.handled.get("test2")).toBe(1);
  
  emitter.off("test", (_arg) => {
